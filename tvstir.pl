...
# 
# tvstir.pl description goes here
...
if ($help) { 
	...
	print "  --write	| -w		Actually move files (Default is to print changes only)\n";
	print "  --directory	| -w		Directory to check for TV shows (Default is pwd)\n";
...
print "\n";
my $matchtb = Text::Table->new("File\n----", "TV Show\n-- ----", "Season\n------");
...
if ($write) {
	foreach my $key ( keys %matched ) {
		...
		rename "$directory/$key","$output/$matched{$key}/Season $tvseason{$key}/$key";
	}
} else {
...
sub getseries {
	...
	if ($name ne 'Dubios' and $name ne 'Error') {
		...
		my $content = get("http://thetvdb.com/api/GetSeries.php?seriesname=$name");
